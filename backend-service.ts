...
import express, { Request as ExpressRequest, Response as ExpressResponse, NextFunction as ExpressNextFunction, ErrorRequestHandler } from 'express';
...
async function processReviewAndRatingTasks(userAddress: string, isReviewTextPresent: boolean) {
    // Always attempt to complete "Rate a dApp" tasks
    ...
    // If review text is present, also attempt "Write a Review" tasks and process streak
    if (isReviewTextPresent) {
        ...
        // Streak logic (only applies if review text was present)
        const profile = await getOrCreateUserProfile(userAddress);
        const now = Date.now();
...
async function startReviewListener(sdk: RateCaster) {
    ...
    try {
        ...
        logger.info('Will attempt to restart review listener in 5 seconds...');
...
export async function startBackendApplication() {
    try {
        ...
        app.get('/api/health', (req: ExpressRequest, res: ExpressResponse) => res.status(200).json({ 
            status: 'UP',
        ...
        app.get('/api/dapps', (req: ExpressRequest, res: ExpressResponse) => { 
            res.json(serializeBigInt(dappsStore));
        });
        
        app.get('/api/dapps/:dappId', (req: ExpressRequest<{ dappId: string }>, res: ExpressResponse) => {
        ...
        app.get('/api/reviews/dapp/:dappId', (req: ExpressRequest<{ dappId: string }>, res: ExpressResponse) => { 
            const reviewsForDapp = reviewsByDappStore[req.params.dappId] || [];
            res.json(serializeBigInt(reviewsForDapp)); 
        });
        
        app.get('/api/stats/dapp/:dappId', (req: ExpressRequest<{ dappId: string }>, res: ExpressResponse) => { 
            const dappId = req.params.dappId;
        ...
        app.get('/api/reviews/user/:userAddress', (req: ExpressRequest<{ userAddress: string }>, res: ExpressResponse) => { 
        ...
        app.get('/api/categories', (req: ExpressRequest, res: ExpressResponse) => {
            try {
        ...
        app.post('/api/actions/wallet-connected', async (req: ExpressRequest<{}, {}, { userAddress: string }>, res: ExpressResponse) => { 
        ...
        app.get('/api/users/profile/:userAddress', async (req: ExpressRequest<{userAddress: string}>, res: ExpressResponse) => { 
        ...
        app.get('/api/leaderboard/top-reviewers', (req: ExpressRequest, res: ExpressResponse) => {
        ...
        app.get('/api/leaderboard/top-streaks', (req: ExpressRequest, res: ExpressResponse) => {
            const leaderboard = Object.values(userProfilesDB)
        ...
        app.get('/api/tasks/active', (req: ExpressRequest, res: ExpressResponse) => {
            const activeTasks = taskDefinitionsStore.filter(t => t.isActive);
            res.json(activeTasks); 
        });

        app.get('/api/tasks/progress', async (req: ExpressRequest<{}, {}, {}, { userAddress?: string }>, res: ExpressResponse) => { 
        ...
        app.post('/admin/tasks/define', async (req: ExpressRequest<{}, {}, TaskDefinition>, res: ExpressResponse) => { 
        ...
        app.post('/api/actions/refresh-dapp-from-chain', async (req: ExpressRequest<{}, {}, { dappId: string }>, res: ExpressResponse) => { 
        ...
        const generalErrorHandler: ErrorRequestHandler = (err: any, req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
